...
module.exports.powershell = function(script, paramsAsString) {
	...
	**/
	...
	exec(command, function(error, stdout, stderr) {
		...
		if (error) {
			deferred.reject({ success: false, status: 400, caller: 'common.js>powershell', data: stderr });
			return;
		}

		deferred.resolve({ success: true, status: 200, caller: 'common.js>powershell', data: stdout });
		return;
	});
...
module.exports.checkapikey = function(apikey) {
	/**
	* Confirms the apikey is proper, and returns a 403 forbidden if it is not
	**/

	if (apikey === 'd1d0d262-2281-429a-afc3-ac0b908c8d5e') {
		return { success: true, status: 200, caller: 'common.js>checkapikey', data: 'Good api key' };
	} else {
		return { success: true, status: 403, caller: 'common.js>checkapikey', data: 'Bad api key' };
	}

	return { success: false, status: 400, caller: 'common.js>checkapikey', data: 'Unknown error' };
}